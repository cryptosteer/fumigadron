...
const App = {
    ...
    networkId: null,
    /* 
    La cuenta de empresa debe ser la misma que realiza el despliegue de los contratos
    En caso de ser nulo es asignado automaticamente el indice [0] de web3.eth.getAccounts()
    Se sugiere que este desbloqueada desde el inicio
    */
    cuentaEmpresa: null, // Si usa truffle develop o ganache dejar en nulo
    /*
    Las cuentas de propietarios pueden ser cualquier otra direccion registrada en el nodo
    Si no esta debloqueada el sistema pedira el password para hacerlo desde la UI
    En caso de ser nulo es asignado automaticamente a los indeces [1 a 3] de web3.eth.getAccounts()
    */
    cuentaPropietario: { // Si usa truffle develop o ganache dejar en nulo
        1: null,
        2: null,
        3: null,
    },
    ...
    start: async function() {
        ...
        try {
            ...
            if ((!this.cuentaEmpresa)&&((cuentas.length > 0)))
                this.cuentaEmpresa = cuentas[0];
            if ((!this.cuentaPropietario[1])&&(cuentas.length > 1))
                this.cuentaPropietario[1] = cuentas[1];
            if ((!this.cuentaPropietario[2])&&(cuentas.length > 2))
                this.cuentaPropietario[2] = cuentas[2];
            if ((!this.cuentaPropietario[3])&&(cuentas.length > 3))
    ...
    obtenerFumi: async function(cuenta) {
        ...
        var txHash;
        try {
            txHash = await obtenerFumi().send(options);
        } catch(e) { 
            console.error(e); 
            this.addEventLog('Error al obtener FUMI.', 'error');
            throw "Error"; 
        }
    ...
    guardarInformacionEmpresa: async function() {
        ...
        var txHash;
        try {
            txHash = await setNombreEmpresa(nombre).send(options);
        } catch(e) { 
            console.error(e); 
            this.addEventLog('Error al guardar el nombre de la empresa.', 'error');
            throw "Error"; 
        }
    ...
    guardarInformacionDron: async function() {
        ...
        try {
            ...
            console.error(e); 
            this.addEventLog('Error al crear el dron.', 'error');
            throw "Error"; 
        }
    ...
    cargarPropietario: async function(propietarioNumero) {
        ...
        if (this.cuentaPropietario[propietarioNumero]) {
            ...
            try {
                ...
                const { existePropietario, crearPropietario, getPropietarioNombre, getNumeroTokens, getTokenIdByIndex, getParcela, getNumeroSolicitudes, isParcelaEnSolicitud } = this.contratoPropietarios.methods;
                const { getBalance } = this.contratoFumiCoin.methods;
                const existe = await existePropietario().call({from: this.cuentaPropietario[propietarioNumero], gas: 500000});
                if (!existe) {
                    var txHash;
                    try {
                        txHash = await crearPropietario().send(options);
                    } catch(e) { 
                        console.error(e); 
                        this.addEventLog('Error al crear el propietario.', 'error');
                        throw "Error"; 
                    }
                ...
                var parcela, existeSolicitud;
                for (var i=0; i<numParcelas; i++) {
                    var id = await getTokenIdByIndex(i).call(options);
                    try {
                        parcela = await getParcela(id).call(options);
                        existeSolicitud = await isParcelaEnSolicitud(id).call(options);
                    } catch(e) {
                        console.error(e);
                    }
                    var controls = "";
    ...
    guardarInformacionPropietario: async function() {
        ...
        var txHash;
        try {
            txHash = await setPropietarioNombre(nombre).send(options);
        } catch(e) { 
            console.error(e); 
            this.addEventLog('Error al guardar el nombre del propietario.', 'error');
            throw "Error"; 
        }
    ...
    guardarInformacionParcela: async function() {
        ...
        try {
            ...
            console.error(e); 
            this.addEventLog('Error al crear la parcela.', 'error');
            throw "Error"; 
        }
    ...
    cargarDronesParaTrabajo: async function(idParcela) {
        ...
        if (listaDrones != "") {
            ...
            $("#dronesParaTrabajo").addClass("d-none");
            $("#noDronesParaTrabajo").removeClass("d-none");            
        }
    },
    ...
    solicitarTrabajo: async function(idParcela, idDron) {
        ...
        const { increaseAllowance } = this.contratoFumiCoin.methods;
        const { getDron } = this.contratoFumigaDron.methods;
        var options = {from: this.cuentaPropietario[this.propietarioActivo], gas: 500000}
        var txHash;
        try {
            txHash = await solicitarTrabajo(idDron, idParcela).send(options);
        } catch(e) { 
            console.error(e); 
            this.addEventLog('Error al solicitar el trabajo.', 'error');
            throw "Error"; 
        }
        ...
        if (eventoTrabajoSolicitado.length > 0) {
            ...
            var dron = await getDron(idDron).call(options);
            var costo = dron[3];
            await increaseAllowance(this.cuentaEmpresa, costo).send(options);
        }
        this.cargarPropietario(this.propietarioActivo);
        this.spinnerHide();
    },
    ...
    realizarTrabajo: async function(idParcela, idDron) {
        ...
        try {
            ...
            console.error(e); 
            this.addEventLog('Error al realizar el trabajo.', 'error');
            throw "Error"; 
        }
